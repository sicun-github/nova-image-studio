...
知意图是一个面向个人/团队的 AI 图像生成工作台。它理解你的创作意图，统一调度图片生成、提示词优化、反推提示词和素材管理流程。前端使用 Next.js 16 + React 19 静态导出（PWA），后端是轻量 Node.js 服务（`server.js` + SQLite + WebSocket），统一调度任务并代理图像生成 API。
